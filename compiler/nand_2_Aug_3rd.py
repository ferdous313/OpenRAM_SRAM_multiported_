...
class nand_2(design.design):
    ...
    def add_rails(self):
        ...
        # Relocate the origin
        self.gnd_position = vector(0, - 8*drc["metal1_to_metal1"])
    ...
    def connect_well_contacts_lima(self):
        ...
        self.add_rect(layer="metal1",
                      offset=offset, 
                      width=drc["minwidth_metal1"],
                      height=self.gnd_height-drc["minwidth_metal1"])
    ...
    def Side_PTX_add_and_create_side_ptx_lima(self):
        ...
        while (i<=self.no_read_only_port):
            ...
            if (i%2!=0):
                ...
                self.add_rect(layer="metal2",
                               offset=[xoffset_RBL_bar_left,viaoffset_side_left_y-self.height-(self.down_ptx_no+self.no_read_only_port)*drc["metal1_to_metal1"]],
                               width=drc["minwidth_metal2"],
                               height=(self.nmos1.height*self.down_ptx_no+1)+2*self.height)
    ...
    def extend_wells_lima(self):
        ...
        pwell_right=self.end_of_ptx_position_right.x+ self.nmos1.width*self.no_read_only_port
        pwell_left=self.end_of_ptx_position_left.x-self.nmos1.width*self.no_read_only_port
        self.pwell_width = pwell_right- pwell_left
        
        #self.pwell_width = self.end_of_ptx_position_right.x-self.end_of_ptx_position_left.x
        offset=vector( pwell_left+self.nmos1.width,self.end_of_ptx_position_left.y-1.1*self.nmos1.height)
	self.add_rect(layer="pwell",
                      offset=offset,
