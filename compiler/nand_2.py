...
class nand_2(design.design):
    ...
    def Side_PTX_add_and_create_side_ptx_lima(self):
        ...
        while (i<=self.no_read_only_port):
            ...
            if (i%2!=0):
                ...
                via_offset = [xoffset_RBL_left, via_side_left_position_y]
                layer_stack_via1 = ["metal1", "via1", "metal2"]   
                self.add_via(layer_stack_via1,via_offset)
                ...
                via_offset = [xoffset_RBL_bar_left, via_RBL_bar_left_position_y]
                layer_stack_via1 = ["metal1", "via1", "metal2"]   
                self.add_via(layer_stack_via1,via_offset)
                ...
                offset=vector(R_Row_ptx_origin+ self.nmos_ptx.active_contact_positions[0].x,  self.nmos_left_side_position_bar.y-self.nmos1.width)         
                self.add_inst(name=name_nmos,
                ...
                height=(4)*drc["metal1_to_metal1"]
                #offset=vector(R_Row_ptx_origin+self.nmos_R_Row_ptx.active_contact_positions[1].x+self.nmos_R_Row_ptx.active_contact.width*.5,offset.y-self.nmos_R_Row_ptx.active_contact.height),
                self.add_rect(layer="metal1",
                               offset=vector(R_Row_ptx_origin+self.nmos_R_Row_ptx.active_contact_positions[1].x+self.nmos_R_Row_ptx.active_contact.width*.5,self.gnd_position.y),
                               width=drc["minwidth_metal1"],
                               height=height)
                #Connect Source of  R-Row ptx to the above side_ptx
                height=(6)*drc["metal1_to_metal1"]
                self.add_rect(layer="metal1",
                               offset=vector(R_Row_ptx_origin+self.nmos_R_Row_ptx.active_contact_positions[0].x+self.nmos_R_Row_ptx.active_contact.width*.42,offset.y-self.nmos_R_Row_ptx.active_contact.height),
                               width=drc["minwidth_metal1"],
                               height=height)

                #connect Poly R-Row Ptx
                poly_position_R_Row_ptx=R_Row_ptx_origin+self.nmos_R_Row_ptx.poly_positions[0].x+drc["minwidth_poly"]
                #height=self.nmos_R_Row_ptx.poly_positions[0].y-
                #R_Row_y=R_Row_y-(i+1.3)*drc["metal1_to_metal1"]+drc["minwidth_metal1"]
               
                #height= abs(R_Row_y+self.nmos_R_Row_ptx.active_position.y+2.6*self.nmos_R_Row_ptx.height)
		      ...
		      width=drc["minwidth_poly"],                        
                      height= height)
                R_Row_y=R_Row_y-2.3*drc["metal1_to_metal1"]-2.3*drc["metal1_to_metal1"]
    ...
    def extend_Q_and_Q_bar_lima(self):
        ...
        while(R_Row_cnt<=self.no_read_only_port):
            ...
            R_Row_cnt=R_Row_cnt+1
            row_y=row_y-drc["metal1_to_metal1"]-1.3*drc["minwidth_metal1"]
